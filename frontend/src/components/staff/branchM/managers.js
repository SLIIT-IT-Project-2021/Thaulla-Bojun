  ...
  printDocument() {  
    ...
    html2canvas(input)  
      .then((canvas) => {
        ...
        var img1 = new Image();
        img1.src = path.resolve('branchpdfM.png');
        var img2 = new Image();
        img2.src = path.resolve('scm.jpg');
        ...
        doc.setFontSize(16);
        doc.text(10, 70, 'Branch Management')
        ...
        const dateStr = "BranchManagement_" + date[0] + date[1] + date[2] + date[3] + date[4];
        doc.save(`report_${dateStr}.pdf`);
      });  
  }  
  
  componentDidMount() {  
    axios.get('http://localhost:8070/branches').then(response => {  
      ...
      <div>  
        <TableContainer id="pdfdiv" className="txt" component={Paper}>  
          <Table stickyHeader aria-label="sticky table">  
            ...
            <TableBody>  
              {  
                this.state.BranchData.map((p, index) => {  
                  return <TableRow key={index}>  
                    <TableCell align="right">{p.name}</TableCell>  
                    <TableCell align="right">{p.city}</TableCell>  
                    <TableCell align="right">{p.branchID}</TableCell>  
                    <TableCell align="right">{p.address}</TableCell>  
                    <TableCell align="right">{p.contactNo}</TableCell>  
