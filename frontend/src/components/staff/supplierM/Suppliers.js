  ...
  printDocument() {  
    ...
    html2canvas(input)  
      .then((canvas) => {
        ...
        const dateStr = "SupplierManagement_" + date[0] + date[1] + date[2] + date[3] + date[4];
        doc.save(`report_${dateStr}.pdf`);
      });  
  }  
  
  componentDidMount() {  
    axios.get('http://localhost:8070/suppliers').then(response => {  
          ...
          <Table stickyHeader aria-label="sticky table">  
            <TableHead>  
              <TableRow> 
                ...
                <TableCell align="right">Prior Experiance</TableCell>  
                <TableCell align="right">Photo</TableCell>  
                <TableCell align="right">Items Purchased</TableCell>
            ...
            <TableBody>  
              {  
                ...
                this.state.SupplierData.map((p, index) => {  
                  return <TableRow key={index}>  
                    <TableCell align="right">{p.supplierID}</TableCell>  
                    <TableCell align="right">{p.fullName}</TableCell>  
                    <TableCell align="right">{p.address}</TableCell>  
                    <TableCell align="right">{p.priorExperiance}</TableCell>  
                    <TableCell align="right">{p.itemsPurchased}</TableCell> 
